<script lang="ts">
    ...
    /**
     * Downloads a meeting summary in the requested format.
     *
     * Sends a POST request to the summary endpoint with the chosen export
     * type. On success the backend returns a file (PDF or CSV) which is
     * automatically downloaded via a temporary anchor element.
     *
     * @param exportType - The format to download ('pdf' or 'csv').
     */
    async function handleDownload(exportType: ExportType) {
        if (exportType === 'pdf') {
        ...
        try {
            const url = `/api/v1/meetings/${meeting.id}/summary?export=${exportType}`;
            ...
            if (!res.ok) {
                if (res.status === 501 && exportType === 'pdf') {
            ...
            toast.success(exportType === 'pdf' ? 'Summary downloaded!' : 'CSV downloaded!');
        } catch (err: any) {
        ...
        >
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Download an AI-powered PDF summary or a CSV of all responses.
                </p>
            </div>
                ...
                {#each sortedQuestions as q, i}
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-medium text-foreground wrap-break-word">
                                {q.prompt}
                            </p>
...
{#if showCopyModal}
    ...
    >
        ...
        >
            ...
            />
            {#if copyError}
                <p class="mt-1 text-(--text-label)">{copyError}</p>
            {/if}
