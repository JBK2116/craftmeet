...
{#if variant === 'inline'}
    <div class="flex h-full flex-col">
        ...
        <div class="flex items-center gap-2 border-b border-border px-4 py-3">
            ...
            <span class="text-sm font-semibold text-[var(--text-heading)]">Chat</span>
            {#if chats.length > 0}
                <span
                    class="ml-auto rounded-full bg-primary px-2 py-0.5 text-[10px] font-bold text-primary-foreground"
                >
                    {chats.length}
                </span>
            {/if}
        ...
        <!-- Messages -->
        <div bind:this={scrollEl} class="flex-1 overflow-y-auto px-3 py-2">
            {#if chats.length === 0}
